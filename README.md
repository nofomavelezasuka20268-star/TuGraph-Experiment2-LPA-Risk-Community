...
通过本次实验，我对 TuGraph 平台和图数据库在区块链数据分析中的作用有了更直观的理解。此前在实验 1 中，我已经完成过比特币交易数据的图建模和数据导入；而本次实验进一步在同一类交易网络上运行图算法，使我更清楚地认识到：图数据库不仅可以用于存储和可视化关系数据，还可以进一步支持路径分析、社区检测、中心性计算等更深入的网络分析任务。

从技术使用角度看，TuGraph 的优势在于能够将原本存储在 CSV 文件中的交易关系转化为直观的图结构。elliptic_txs_classes.csv 和 elliptic_txs_edgelist.csv 在表格中只是交易编号、类别和边列表；导入 TuGraph 后，它们被转化为 Transaction 节点和 Flow_to 边组成的交易网络。通过 Cypher 查询，可以直接观察交易之间的资金流向；通过 Python 存储过程，可以在图数据库内部运行社区检测算法。这种“图建模—图查询—图算法”的完整流程，使区块链交易网络不再只是静态表格，而成为可以被分析、解释和挖掘的网络结构。

本次实验中，最有收获的部分是将 LPA 社区检测与链上反洗钱问题结合起来。过去我对区块链数据的理解更多停留在交易记录、地址、哈希等概念层面；通过这次实验，我认识到链上交易分析还可以从网络结构角度展开。例如，一个 unknown 交易节点本身未必能说明风险，但如果它与多个已知非法交易处于同一个高凝聚度社区中，就具有进一步审查价值。这让我更加理解图算法在区块链风控、交易所合规、地址画像和异常交易筛查中的应用潜力。

作为金融学院跨选这门课的同学，我在信息技术基础方面并不占优势，尤其是刚开始接触 TuGraph、Docker、存储过程和图算法时，会遇到不少理解和操作上的困难。但王老师在课堂上讲解很耐心，会给我们录像供事后参考。课间提问时也都有问必答，这对跨专业的学生非常重要。课程本身既涉及区块链与数字货币的金融问题，也涉及图数据库、图算法等技术方法，对金融背景的学生来说有一定挑战，但也提供了一个很好的交叉学习机会。

这一学期学院和相关领导多次关注课程建设，也能感受到信息学院对这门课的重视。对我而言，这门课的价值不只是学习某一个具体软件或算法，而是提供了一种从数据结构、网络关系和实际业务问题出发理解区块链的方式。相比单纯讲概念，实验环节能够帮助学生真正把区块链交易数据“跑起来”“看出来”“分析出来”。

如果对课程提出一点建议，我认为后续可以进一步增强课堂互动和案例讨论。例如，在讲解 PageRank、LPA、Node2Vec 等算法时，可以加入更多与链上反洗钱、交易所风控、数字货币监管、地址聚类等实际场景相关的小问题，让学生在课堂上先讨论“这个算法可以解决什么业务问题”，再进入代码和平台操作。这样既能帮助非信息专业学生建立直觉，也能让实验结果更容易与课程主题联系起来。

另外，王老师平时上课时老调侃自己英文不好、记忆力不好，但从课堂组织和答疑过程看，实际并不是这样。老师对课程内容、实验步骤和学生问题都比较熟悉，也能在学生遇到问题时快速定位可能原因。作为跨学院选课的学生，我能感受到老师对这门课和学生学习体验的投入。希望老师在后续授课中可以更自信一些，也可以继续保持这种耐心、开放的教学风格。如果以后还有机会选到王老师相关的课程，我也会愿意继续学习。
